...
@PIPELINES.register_module()
class NormalizeIR(object):
    """Normalize the image.

    Added key is "img_norm_cfg".

    Args:
        mean (sequence): Mean values of 1 channel.
        std (sequence): Std values of 1 channel.
...
@PIPELINES.register_module()
class NormalizeRGBT(object):
    """Normalize the image.

    Added key is "img_norm_cfg".

    Args:
        mean (sequence): Mean values of 4 channels.
        std (sequence): Std values of 4 channels.
        to_rgb (bool): Whether to convert the image from BGR to RGB,
            default is true.
    """

    def __init__(self, mean, std, meanIR, stdIR, to_rgbt=True):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        # self.meanIR = np.array(meanIR, dtype=np.float32)
        # self.stdIR = np.array(stdIR, dtype=np.float32)     
        self.meanIR =meanIR
        self.stdIR=stdIR
        self.to_rgbt = to_rgbt
    ...
    def __call__(self, results):
        ...
        imgIR=img[:,:,3]
        im = Image.fromarray(imgIR)
      

        stdinvIR = 1 /self.stdIR
        # inplace

        imgIR=imgIR-self.meanIR
        imgIR=imgIR*stdinvIR  # inplace

        imgRGB=img[:,:,:3]
        imgRGB = mmcv.imnormalize(imgRGB, self.mean, self.std,
                                          self.to_rgbt)
        img[:,:,3]=imgIR
        img[:,:,:3]=imgRGB
        ...
        #results['pad_shape']=img.shape
        results['img_norm_cfg'] = dict(
            mean=self.mean, std=self.std,meanIR=self.meanIR,stdIR=self.stdIR,to_rgbt=self.to_rgbt)
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(mean={self.mean}, std={self.std}, to_rgbt=' \
                    f'{self.to_rgb})'
        return repr_str
