...
model = dict(
    ...
    pretrained=None,
    backbone=dict(
        type='SimpleCrossMixVisionTransformer',
        in_channels_ir=1,
        in_channels_rgb=3,
        embed_dims=64,
        num_stages=4,
...
checkpoint_config = dict(by_epoch=False, interval=5, max_keep_ckpts=50)
evaluation = dict(interval=25, metric='mIoU', pre_eval=True, save_best='mIoU')
#checkpoint = 'earlyfuseformer_pretrained.pth'
work_dir = './work_dirs/SimpleCross_attn_earlyfuseformer_mit-b2_4k_32_mfnet_focal_rgbt'
#gpu_ids = [1]
auto_resume = False
