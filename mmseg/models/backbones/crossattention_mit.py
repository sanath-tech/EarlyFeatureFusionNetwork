...
class CrossEfficientMultiheadAttention(MultiheadAttention):
    ...
    def forward(self, x,y, hw_shape, identity=None):

        x_q = x
        if self.sr_ratio > 1:
            x_kv = nlc_to_nchw(y, hw_shape)
            x_kv = self.sr(x_kv)
    ...
    def legacy_forward(self, x,y, hw_shape, identity=None):
...
class CrossTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        # The ret[0] of build_norm_layer is norm name.
        self.norm1_rgb = build_norm_layer(norm_cfg, embed_dims//2)[1]
        self.norm1_ir = build_norm_layer(norm_cfg, embed_dims//2)[1]

        self.attn_rgb = CrossEfficientMultiheadAttention(
        ...
        self.attn_ir = CrossEfficientMultiheadAttention(
        embed_dims=embed_dims//2,
        num_heads=num_heads,
        attn_drop=attn_drop_rate,
        proj_drop=drop_rate,
    ...
    def forward(self, x,y, hw_shape):
            ...
            x = self.norm1_rgb(x[0])
            y = self.norm1_rgb(y[0])
            x = self.attn_rgb(x,y, hw_shape, identity=x)
            y = self.attn_ir(y,x, hw_shape, identity=y)
...
class CrossMixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.crossattention = CrossTransformerEncoderLayer(
                        embed_dims=embed_dims,
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layersRGB):
            ...
            if i == 0:
                xrgb = self.patch_embed_rgb(xrgb)
                xir = self.patch_embed_rgb(xir)
                xrgb = self.crossattention(xrgb, xir,hw_shape)  
                xrgb = nlc_to_nchw(xrgb, hw_shape)          
