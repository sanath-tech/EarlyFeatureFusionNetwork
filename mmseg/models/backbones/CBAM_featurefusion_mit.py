...
class CBAMFeatureFusionMixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        if self.teacher_attention==True:
            ...
            self.cbam3=CBAM(gate_channels=640)
            self.cbam4=CBAM(gate_channels=1024)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layersRGB):
            ...
            if self.teacher_attention==True:
                if i==1:
                    xrgb = self.cbam2(torch.cat([xrgb, outsIR[i]], dim=1))
                if i==2:
                    xrgb = self.cbam3(torch.cat([xrgb, outsIR[i]], dim=1))
                if i==3:
                    xrgb = self.cbam4(torch.cat([xrgb, outsIR[i]], dim=1))
            if i in self.out_indices:
                outsRGB.append(xrgb)
