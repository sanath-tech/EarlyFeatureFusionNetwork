...
class CBAMMidFusionMixVisionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 with_cp=False):
        super(CBAMMidFusionMixVisionTransformer, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        if self.init_cfg is None:
            ...
            super(CBAMMidFusionMixVisionTransformer, self).init_weights()
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layersRGB):
            ...
            if i == 1:
                xrgb = self.cbam2(torch.cat([xrgb, outsIR[1]], dim=1)) 
            if i in self.out_indices:
                if i==0:
                    outsRGB.append(self.cbam1(torch.cat([xrgb, outsIR[0]], dim=1)))
                else : 
                    outsRGB.append(xrgb)
