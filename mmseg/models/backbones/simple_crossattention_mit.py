...
class CrossEfficientMultiheadAttention(MultiheadAttention):
    ...
    def forward(self, x,y, hw_shape, identity=None):
        ...
        if self.sr_ratio > 1:
            ...
            x_v = nlc_to_nchw(identity, hw_shape)
            x_v = self.sr(x_v)
            x_v = nchw_to_nlc(x_v)
            x_v = self.norm(x_v)
        else:
        ...
        out = self.attn(query=x_q, key=x_kv, value=x_v)[0]
...
class CrossTransformerEncoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.attn = CrossEfficientMultiheadAttention(
        ...
        self.fuser1=ConvFuser([64,64],64)

    

    def forward(self, x,y, hw_shape):
            z = self.fuser1(x,y)
            z = self.attn(x,y, hw_shape, identity=z)
            z = self.norm2(z)
...
class SimpleCrossMixVisionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 with_cp=False):
        super(SimpleCrossMixVisionTransformer, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        if self.init_cfg is None:
            ...
            super(SimpleCrossMixVisionTransformer, self).init_weights()
