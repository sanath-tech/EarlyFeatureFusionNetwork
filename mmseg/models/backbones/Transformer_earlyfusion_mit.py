...
class SelfAttentionEarlyFusionMixVisionTransformer(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 with_cp=False):
        super(SelfAttentionEarlyFusionMixVisionTransformer, self).__init__(init_cfg=init_cfg)
        ...
        for i, num_layer in enumerate(num_layers0):
            embed_dims_i = (embed_dims * num_heads[i])//2
        ...
        in_channels_rgb = in_channels_rgb*2

        cur = 0
        self.layersFusion = ModuleList()
        for i, num_layer in enumerate(num_layers):
                ...
                # The ret[0] of build_norm_layer is norm name.
                norm = build_norm_layer(norm_cfg, embed_dims_i)[1]
                self.layersFusion.append(ModuleList([patch_embed, layer, norm]))
                cur += num_layer

        self.fuser1=ConvFuser([32,32],64)
    ...
    def init_weights(self):
        if self.init_cfg is None:
            ...
            super(SelfAttentionEarlyFusionMixVisionTransformer, self).init_weights()
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layersRGB):
            ...
            xrgb = self.fuser1([xrgb, xir])


        for i, layer in enumerate(self.layersFusion):
                xrgb, hw_shape = layer[0](xrgb)
                for block in layer[1]:
                    xrgb = block(xrgb, hw_shape)
                xrgb = layer[2](xrgb)
